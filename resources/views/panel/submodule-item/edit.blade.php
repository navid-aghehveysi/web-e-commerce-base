...
{{--    @if( auth()->user()->can('create' , \App\Models\Module::class))--}}
        ...
        <div class="simple-form">
            ...
            <form action="{{ route('panel.submodule-item.update', $submoduleItem->id) }}" method="post"  id="form"
                ...
                <section class="grid grid-cols-6 gap-y-8 gap-x-4 mb-4">
                    <div class="w-full col-span-6 lg:col-span-2">
                        ...
                        <input type="text" name="name_en" id="name_en"
                               value="{{ old('name_en', $submoduleItem->name_en) }}">
                        @error('name_en')
                    ...
                    <div class="w-full col-span-6 lg:col-span-2">
                        ...
                        <input type="text" name="name_fa" id="name_fa"
                               value="{{ old('name_fa', $submoduleItem->name_fa) }}">
                        @error('name_fa')
                        <span class="text-rose-600" role="alert">
                            {{$message}}
                        </span>
                        ...
                        <select id="submodule_id" name="submodule_id" class="">
                            @foreach($submodules as $submodule)
                                <option value="{{$submodule->id}}"
                                    {{ old('submodule_id', $submoduleItem->submodule->name_fa) }}
                                >
                                    {{$submodule->name_fa}}
                                </option>
                            @endforeach
                    ...
                    <div class="w-full col-span-6 lg:col-span-2">
                        ...
                        <select id="status" name="status" class="">
                            @foreach($statuses as $value => $label)
                                <option value="{{$value}}"
                                    {{ old('status',$submoduleItem->status) == $value ?'selected': '' }}
                                >
                                    {{ $label }}
                                </option>
                            @endforeach
